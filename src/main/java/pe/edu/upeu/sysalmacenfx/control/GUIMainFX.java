        ...
        for (MenuMenuItenTO mmix : lista) {
            if (!mmix.menunombre.equals(menuN)) {
                ...
                if (!mmix.menuitemnombre.equals("")) {
                    ...
                    menuItem[menuitem].setId(mmix.nombreObj);
...
// Layout principal
    ...
    class MenuItemListener {
        public void handle(javafx.event.ActionEvent e) {
            if (((MenuItem) e.getSource()).getId().equals("miregpelicula")) {
                tabPaneFx.getTabs().clear();
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/main_pelicula.fxml"));
            ...
            if (((MenuItem) e.getSource()).getId().equals("mireggenero")) {
                tabPaneFx.getTabs().clear();

                FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/main_genero.fxml"));
                loader.setControllerFactory(context::getBean);
                Parent paneFromFXML;
                try {
                    paneFromFXML = loader.load();
                    ScrollPane dd = new ScrollPane(paneFromFXML);
                    Tab clienteTab = new Tab("Reg. Genero", dd);
                    tabPaneFx.getTabs().add(clienteTab);
                } catch (IOException ex) {
                    throw new RuntimeException(ex);
                }
            }
            if (((MenuItem) e.getSource()).getId().equals("miregactor")) {
                tabPaneFx.getTabs().clear();

                FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/main_actor.fxml"));
                loader.setControllerFactory(context::getBean);
                Parent paneFromFXML;
                try {
                    paneFromFXML = loader.load();
                    ScrollPane dd = new ScrollPane(paneFromFXML);
                    Tab clienteTab = new Tab("Reg. Actor", dd);
                    tabPaneFx.getTabs().add(clienteTab);
                } catch (IOException ex) {
                    throw new RuntimeException(ex);
                }
